...
if __name__ == "__main__":
    ...
    print("\t\t\t\t\t<<<<<<< WELCOME TO THE COEP LIBRARY >>>>>>>\n")
    ...
    while (True):
        try:
            ...
            if usr_response == 1:  
                COEPlibrary.displayAvailableBooks()
            elif usr_response == 2:  
                COEPlibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  
                COEPlibrary.returnBook(student.returnBook())
            elif usr_response == 4:  
                COEPlibrary.donateBook(student.donateBook())
            elif usr_response == 5:  
                for i in track:
